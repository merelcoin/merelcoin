...
const char * const BITCOIN_CONF_FILENAME = "merelcoin.conf";
const char * const BITCOIN_PID_FILENAME = "merelcoin.pid";
...
#else
    const char* pszModule = "merelcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MerelCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MerelCoin";
#else
    // Unix
    return pathRet / ".merelcoin";
#endif
#endif
}
